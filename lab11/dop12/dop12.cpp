...
int Count(Tree* t);               //Подсчет количества узлов с четными ключами
...
void main()
{
	...
	for (;;)
	{
		...
		cout << "8 -	подсчет количества узлов с четными ключами\n";
		cout << "0 -	выход\n";
		cout << "ваш выбор?\n";
		cin >> choice;
		cout << "\n";
		...
		case 8: {
			...
			if (Count(Root) == 0) {
				cout << "Узлом с четными ключами нет" << endl;
			}
			else {
				cout << "Количество узлов с четными ключами: " << Count(Root) << endl;
