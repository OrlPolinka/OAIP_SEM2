...
int Count(Tree* t);               //Подсчет количества листьев дерева 
...
void main()
{
	...
	for (;;)
	{
		...
		cout << "8 -	подсчет количества листьев дерева\n";
		cout << "0 -	выход\n";
		cout << "ваш выбор?\n";
		cin >> choice;
		cout << "\n";
		...
		case 8: {
			...
			if (Count(Root) == 0) {
				...
				cout << "Количество листьев дерева: " << Count(Root) << endl;
...
int Count(Tree* p1) {
	if (p1 != NULL) {
		...
		if (p1->Left == NULL && p1->Right == NULL) {
			// Если обе дочерние вершины равны NULL, то это лист
			return 1;
		}
