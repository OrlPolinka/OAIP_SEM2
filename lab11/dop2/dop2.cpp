...
int Count(Tree* t);               //Подсчет количества листьев дерева, являющихся правыми дочерними вершинами
...
void main()
{
	...
	for (;;)
	{
		...
		cout << "8 -	подсчет количества листьев дерева, являющихся правыми дочерними вершинами\n";
		cout << "0 -	выход\n";
		cout << "ваш выбор?\n";
		cin >> choice;
		cout << "\n";
		...
		case 8: {
			...
			if (Count(Root) == 0) {
				cout << "Нет листьев дерева, являющихся правыми дочерними вершинами." << endl;
			}
			else {
				cout << "Количество листьев дерева, являющихся правыми дочерними вершинами: " << Count(Root) << endl;
...
int Count(Tree* p1) {
	if (p1 == nullptr) {
		return 0; // Дерево пустое
	}

	int LeftCount = Count(p1->Left);
	int RightCount = Count(p1->Right);

	// Если вершина является правым листом, увеличиваем счетчик на 1
